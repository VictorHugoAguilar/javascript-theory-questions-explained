...
Se programa una actualización cuando una o más propiedades cambian o cuando se llama a requestUpdate().
La actualización se realiza antes de que se pinte el siguiente cuadro.
Se establecen los atributos reflectantes.
Se llama al método de renderizado del componente para actualizar su DOM interno.
La actualización se completa y la promesa updateComplete se resuelve.
...
Muchos métodos de actualización reactiva reciben un mapa de propiedades modificadas. Las claves del mapa son los nombres de propiedad y sus valores son los valores de propiedad anteriores. Siempre puede encontrar los valores de propiedad actuales usando this.property o this[property].

**Tipos de TypeScript para propiedades modificadas**
Si está usando TypeScript y desea una verificación de tipos sólida para el mapa de propiedades modificadas, puede usar PropertyValues<this>, que infiere el tipo correcto para cada nombre de propiedad.
...
Cambiar una propiedad durante la actualización (hasta el método render() incluido) actualiza el mapa de propiedades modificadas, pero no desencadena una nueva actualización. Cambiar una propiedad después de render() (por ejemplo, en el método updated()) desencadena un nuevo ciclo de actualización, y la propiedad modificada se agrega a un nuevo mapa de propiedades modificadas para usarse en el próximo ciclo.
...
Se llama cuando se establece una propiedad reactiva. De forma predeterminada, hasChanged() realiza una verificación de igualdad estricta y, si devuelve verdadero, se programa una actualización. Consulte configuración de hasChanged() para obtener más información.
...
La lista de propiedades que han cambiado se almacena en un mapa de propiedades modificadas que se pasa a los métodos de ciclo de vida posteriores. Las claves del mapa son los nombres de propiedad y sus valores son los valores de propiedad anteriores.

Opcionalmente, puede pasar un nombre de propiedad y un valor anterior al llamar a `requestUpdate()`, que se almacenará en el mapa de propiedades modificadas. Esto puede ser útil si implementa un getter y setter personalizado para una propiedad. Consulte Propiedades reactivas para obtener más información sobre la implementación de captadores y definidores personalizados.
...
Si shouldUpdate() devuelve verdadero, lo que hace de manera predeterminada, entonces la actualización continúa con normalidad. Si devuelve falso, no se llamará al resto del ciclo de actualización, pero la promesa de actualización completa aún se resuelve.

Puede implementar shouldUpdate() para especificar qué cambios de propiedad deben generar actualizaciones. Utilice el mapa de propiedades modificadas para comparar los valores actuales y anteriores.
...
También es posible que desee utilizar una prueba/captura si está esperando la promesa updateComplete:
